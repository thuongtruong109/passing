...
const Toggle: React.FC<Props> = ({ dataInputId, dataStatusId }: Props) => {
  const btnRef = useRef<HTMLButtonElement>(null);

  const [viewable, setViewable] = React.useState(false);
  const toggleViewable = () => setViewable(!viewable);
  ...
  useEffect(() => {
    ...
    btn.addEventListener("click", () => {
      if (input.type === "password" && viewable) {
  ...
  }, [dataInputId, dataStatusId, viewable]);

  return (
    <button type="button" ref={btnRef} onClick={toggleViewable}>
      <svg
